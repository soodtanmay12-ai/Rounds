  ...
  event.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      for (const client of clientList) {
        if ('focus' in client) {
          client.focus();
          if (dest) client.postMessage({ type: 'notification-click', dest });
          return;
        }
      }
...
async function checkDailyStatus() {
  ...
  if (status.dateKey === todayKey && hour >= 18 && status.mcqCount < status.mcqTarget) {
    const remaining = status.mcqTarget - status.mcqCount;
    await self.registration.showNotification('Rounds', {
      body: `${remaining} MCQ${remaining !== 1 ? 's' : ''} left for today's ${status.mcqTarget}-question target.`,
      icon: './apple-touch-icon.png',
      badge: './favicon-32.png',
      tag: 'rounds-periodic-mcq',
      // Matches index.html's showLocalNotification for the same nudge --
      // an undismissed notification is what actually drives Android's
      // automatic home-screen icon badge (Chrome's own Badging API docs
      // confirm the API itself is a no-op on Android).
      requireInteraction: true
    });
  }
}

// Mirrors mirrorStatusToIndexedDB() in index.html -- same DB/store/key
// names, read-only here. A service worker runs in its own global context
// and can't reach the page's variables or localStorage, only
// IndexedDB/Cache, hence this tiny separate snapshot instead of the real
// app data.
function readStatus() {
